...
```math
\vec{x}_{k+1} = \vec{x}_k - \alpha_k H_k^{-1} \nabla f(\vec{x}_k)
```
...
Let's refer to this approximate Hessian as $`B_k`$[^1]. Furthermore, following the derivation in Nocedal and Wright, let's write the second-order Taylor expansion of our function at the point $`\vec{x}_k`$. I'll use $`f_k \equiv f(\vec{x}_k)`$ to represent the objective function and $`\vec{g}_k \equiv \nabla f(\vec{x}_k)`$ to represent the gradient.
...
```math
\begin{align}\vec{p}_k^{\intercal} \vec{g}_{k+1} &\geq c_2(\vec{p}_k^{\intercal} \vec{g}_k) \\\\ \vec{s}_k^{\intercal} \vec{g}_{k+1} &\geq c_2(\vec{s}_k^{\intercal} \vec{g}_k) \\\\ \vec{s}_k^{\intercal} (\vec{y}_k + \vec{g}_{k}) &\geq c_2(\vec{s}_k^{\intercal} \vec{g}_k) \\\\ \vec{s}_k^{\intercal} \vec{y}_k + \vec{s}_k^{\intercal} \vec{g}_{k} &\geq c_2(\vec{s}_k^{\intercal} \vec{g}_k) \\\\ \vec{s}_k^{\intercal} \vec{y}_k &\geq (c_2 - 1)(\vec{s}_k^{\intercal} \vec{g}_k) \\\\ \vec{s}_k^{\intercal} \vec{y}_k &\geq (c_2 - 1)\alpha_k \vec{p}_k^{\intercal} \vec{g}_k \end{align}
```

Since we require $`c_2 < 1`$ and $`\alpha_k > 0`$, and since $`\vec{p}_k`$ is a descent direction, both factors on the right-hand side are negative. Their product is therefore positive, so the curvature condition holds.
